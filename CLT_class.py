...
class CLT:
    ...
    '''
    def update(self, dataset, weights , r=None):
        # print("Updating nowwwwww")
        ...
        # print("weights in clt class - " , weights)
        ...
        # Implementing this for Random forest. If the parameter 'r' is passed while calling this function then, this will run for Q3 ie Random Forest
        if r:
            for x in range(r):
                row = np.random.randint(dataset.shape[1])
                col = np.random.randint(dataset.shape[1])
                edgemat[row][col] = 0
                # edgemat[col][row] = 0
